...
import { fetchUserRole } from "./config/firebase";
import "./App.css";
...
const App = () => {
  ...
  if (!isAuthChecked) {
    return <div>Loading...</div>; // Prevent rendering until auth state is determined
  }

  if (isLoadingRole) {
    return <div>Loading...</div>; // Prevent rendering until role is determined
  }
